...
**** noone =(\ist, &pred = {$_} --> Bool:D)=
Check if none of the elements (noone, not any) satisfies the given predicate. It has a short-circuiting behaviour, i.e., if a satisfactory element is found, traversal immediately stops. `none` would be a better name but it's taken.
#+begin_src raku
